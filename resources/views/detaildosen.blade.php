...
<head>
    ...
    <title>Detail Dosen - ITB STIKOM BALI</title>
...
<nav class="navbar navbar-expanded-sm bg-primary fixed-top">
        <div class="container">
        <a href="/tampilkandosen" data-target="nav-mobile" class="sidenav-trigger show-on-large">
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom container">
        <h1><font style="font-family:verdana;font-size:20pt;color:black;">Detail Dosen</font></h1>
    </div>
    ...
    <div class="container">
        ...
        <div class="card" style="max-width: 540px;">
            <input type="hidden" name="id" id="id" value="{{$dosen->id}}">
                <div class="col-md-4">
			        <img src="{{asset('uploads/dosen/'. $dosen->image)}}" width="100%" height="100%" alt="image">
                </div>
			<div class="card-body">
				<h5>{{$dosen->nama_lengkap}}</h5>
                <p>Jenis Kelamin : {{$dosen->jenis_kelamin}}</p>
				<p>Tempat Tanggal Lahir : {{$dosen->tempat_lahir}} , {{$dosen->tanggal_lahir}}</p>
				<p>Pendidikan Terakhir : {{$dosen->pendidikan_terakhir}}</p>
                <p>Alamat : {{$dosen->alamat}}</p>
                <p>No.Telepon : {{$dosen->no_telepon}}</p>
                <p>No.Handphone : {{$dosen->no_handphone}}</p>
